...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scrape_brand.py <url>")
        sys.exit(1)
    ...
    try:
        ...
        output_path = "tmp/brand_data.json"
        with open(output_path, "w") as f:
            json.dump(result_dict, f, indent=2, default=str)
            
        print(f"Successfully extracted brand data to {output_path}")
        # Print JSON to stdout so it can be captured by the calling agent
